...
var $author$project$GradeSchool$update = F2(
	function (toAddStudent, maybeStudent) {
		if (maybeStudent.$ === 'Just') {
			var student = maybeStudent.a;
			return $elm$core$Maybe$Just(
				$elm$core$List$sort(
					A2($elm$core$List$cons, toAddStudent, student)));
		} else {
			return $elm$core$Maybe$Just(
				_List_fromArray(
					[toAddStudent]));
...
var $author$project$GradeSchool$addStudent = F3(
	function (grade, student, school) {
		return A3(
			$elm$core$Dict$update,
			grade,
			$author$project$GradeSchool$update(student),
			school);
	});
var $author$project$GradeSchool$allStudents = function (school) {
	return $elm$core$Dict$toList(school);
};
var $author$project$GradeSchool$empty = $elm$core$Dict$empty;
...
var $author$project$GradeSchool$studentsInGrade = F2(
	function (grade, school) {
		var _v0 = A2($elm$core$Dict$get, grade, school);
		if (_v0.$ === 'Nothing') {
			return _List_Nil;
		} else {
			var studentRoster = _v0.a;
		...
		[
			...
			function (_v0) {
				return A2(
					...
					_List_fromArray(
						['Aimee']),
					A2(
						$author$project$GradeSchool$studentsInGrade,
						2,
						A3($author$project$GradeSchool$addStudent, 2, 'Aimee', $author$project$GradeSchool$empty)));
			}),
			A2(
			$elm_explorations$test$Test$test,
			...
			function (_v1) {
				return A2(
					...
					A2(
						...
						2,
						A3(
							$author$project$GradeSchool$addStudent,
							2,
							'Paul',
							A3(
								$author$project$GradeSchool$addStudent,
								2,
								'Blair',
								A3($author$project$GradeSchool$addStudent, 2, 'James', $author$project$GradeSchool$empty)))));
			}),
			A2(
			$elm_explorations$test$Test$test,
				...
				return A2(
					...
					function () {
						var school = A3(
							...
							7,
							'Logan',
							A3($author$project$GradeSchool$addStudent, 3, 'Chelsea', $author$project$GradeSchool$empty));
						return _List_fromArray(
							[
								A2($author$project$GradeSchool$studentsInGrade, 3, school),
								A2($author$project$GradeSchool$studentsInGrade, 7, school)
							]);
					}());
			}),
			A2(
					...
					A2(
						...
						A3(
							...
							A3(
								...
								5,
								'Bradley',
								A3($author$project$GradeSchool$addStudent, 5, 'Franklin', $author$project$GradeSchool$empty)))));
			}),
			A2(
			$elm_explorations$test$Test$test,
			...
			function (_v4) {
				return A2(
					...
					$author$project$GradeSchool$allStudents(
						A3(
							$author$project$GradeSchool$addStudent,
							1,
							'Charlie',
									...
									A3(
										...
										A3(
											...
											A3(
												...
												1,
												'Anna',
												A3($author$project$GradeSchool$addStudent, 2, 'Peter', $author$project$GradeSchool$empty)))))))));
			}),
			A2(
			$elm_explorations$test$Test$test,
			...
			function (_v5) {
				return A2(
					$elm_explorations$test$Expect$equal,
					_List_Nil,
					A2($author$project$GradeSchool$studentsInGrade, 1, $author$project$GradeSchool$empty));
			})
		]));
var $author$project$Test$Generated$Main2581158782$main = A2(
	$author$project$Test$Runner$Node$run,
	{
		paths: _List_fromArray(
			['/Users/carterweinberg/Exercism/elm/grade-school/tests/Tests.elm']),
		processes: 12,
		report: $author$project$Test$Reporter$Reporter$ConsoleReport($author$project$Console$Text$UseColor),
		runs: $elm$core$Maybe$Nothing,
		seed: 280228770229978
	},
...
_Platform_export({'Test':{'Generated':{'Main2581158782':{'init':$author$project$Test$Generated$Main2581158782$main($elm$json$Json$Decode$int)(0)}}}});}(this));
return this.Elm;
})({});
var pipeFilename = "/tmp/elm_test-93160.sock";
