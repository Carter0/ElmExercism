...
var $author$project$MatchingBrackets$bracketToLookFor = function (_char) {
	return _Utils_eq(
		_char,
		_Utils_chr('{')) ? _Utils_chr('}') : (_Utils_eq(
		_char,
		_Utils_chr('(')) ? _Utils_chr(')') : _Utils_chr(']'));
};
...
var $author$project$MatchingBrackets$isEndingBrackets = function (_char) {
	return _Utils_eq(
		_char,
...
var $author$project$MatchingBrackets$isStartingBrackets = function (_char) {
	return _Utils_eq(
		_char,
		_Utils_chr('{')) || (_Utils_eq(
		_char,
...
var $author$project$MatchingBrackets$updateStack = F2(
	function (bracket, bracketStack) {
		if ($author$project$MatchingBrackets$isStartingBrackets(bracket)) {
			return $elm$core$Result$Ok(
				A2($elm$core$List$cons, bracket, bracketStack));
		} else {
			if ($author$project$MatchingBrackets$isEndingBrackets(bracket)) {
				var popValue = $elm$core$List$head(bracketStack);
				if (popValue.$ === 'Nothing') {
					return $elm$core$Result$Err('Nothing to pop off in stack');
				} else {
					var a = popValue.a;
					return _Utils_eq(
						$author$project$MatchingBrackets$bracketToLookFor(a),
						bracket) ? $elm$core$Result$Ok(
						A2($elm$core$List$drop, 1, bracketStack)) : $elm$core$Result$Err('Brackets do not match');
				}
			} else {
				return $elm$core$Result$Ok(bracketStack);
			}
		}
	});
var $author$project$MatchingBrackets$stackFunctionality = F2(
	function (bracket, bracketStack) {
		return A2(
			$elm$core$Result$andThen,
			$author$project$MatchingBrackets$updateStack(bracket),
			bracketStack);
	});
var $author$project$MatchingBrackets$isPaired = function (input) {
	var _v0 = A3(
		$elm$core$String$foldl,
		$author$project$MatchingBrackets$stackFunctionality,
	...
	if (_v0.$ === 'Err') {
		...
		var value = _v0.a;
		return $elm$core$List$isEmpty(value);
	}
};
		...
		[
			...
			function (_v0) {
				return A2(
					...
					true,
					$author$project$MatchingBrackets$isPaired('[]'));
			}),
			A2(
			$elm_explorations$test$Test$test,
			'empty string',
			...
			function (_v13) {
				return A2(
					$elm_explorations$test$Expect$equal,
					true,
					$author$project$MatchingBrackets$isPaired('(((185 + 223.85) * 15) - 543)/2'));
			}),
			A2(
			$elm_explorations$test$Test$test,
			...
			function (_v14) {
				return A2(
					$elm_explorations$test$Expect$equal,
					true,
					$author$project$MatchingBrackets$isPaired('\\left(\\begin{array}{cc} \\frac{1}{3} & x\\\\ \\mathrm{e}^{x} &... x^2 \\end{array}\\right)'));
			})
		]));
var $author$project$Test$Generated$Main1293483567$main = A2(
	$author$project$Test$Runner$Node$run,
	{
		paths: _List_fromArray(
			['/Users/Carter/Exercism/elm/matching-brackets/tests/Tests.elm']),
		processes: 8,
		report: $author$project$Test$Reporter$Reporter$ConsoleReport($author$project$Console$Text$UseColor),
		runs: $elm$core$Maybe$Nothing,
		seed: 297003073104309
	},
...
_Platform_export({'Test':{'Generated':{'Main1293483567':{'init':$author$project$Test$Generated$Main1293483567$main($elm$json$Json$Decode$int)(0)}}}});}(this));
return this.Elm;
})({});
var pipeFilename = "/tmp/elm_test-4035.sock";
